        ...
        if (toLeft)
        {
            for (int i = size; i >= 0; i--)
            {
                ...
                if (cauldronsObject[(gameObjectID + 1) % cauldronsObject.Count].transform.position.x < Screen.width / 2)
                {
                    ...
                    cauldronsObject[gameObjectID].transform.position = new Vector3(Screen.width / 2 - 1000, cauldronsObject[gameObjectID].transform.position.y, 0);
                    cauldronsObject[(gameObjectID + cauldronsObject.Count - 1) % cauldronsObject.Count].transform.position = new Vector3(Screen.width / 2 + 1000, cauldronsObject[(gameObjectID + cauldronsObject.Count - 1) % cauldronsObject.Count].transform.position.y, 0);
                ...
                clocksObject[i].transform.Translate(-moveSpeed * 0.4f * Time.deltaTime, 0, 0);
        ...
        if (toRight)
        {
            for (int i = 0; i <= size; i++)
            {
                ...
                if (Screen.width / 2 < cauldronsObject[(gameObjectID + cauldronsObject.Count - 1) % cauldronsObject.Count].transform.position.x)
                {
                    ...
                    cauldronsObject[(gameObjectID + 1) % cauldronsObject.Count].transform.position = new Vector3(Screen.width / 2 - 1000, cauldronsObject[(gameObjectID + 1) % cauldronsObject.Count].transform.position.y, 0);
                    cauldronsObject[gameObjectID].transform.position = new Vector3(Screen.width / 2 + 1000, cauldronsObject[gameObjectID].transform.position.y, 0);
